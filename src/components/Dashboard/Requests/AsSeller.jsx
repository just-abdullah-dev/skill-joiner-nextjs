...
export default function AsClient() {
  ...
  useEffect(() => {
    ...
    const get = () => {
      ...
      fetch(
        `/api/servicerequest/freelancer/searchRequests?keyword=${keyword}`,
        requestOptions
      )
  ...
  const handleReq = (id, type) => {
    ...
    fetch(`/api/servicerequest/isAccepted`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
        if (result?.success) {
                      ...
                      <div className=" flex ">
                        ...
                        <div className=" w-4/5">
                          ...
                          {!item?.isAccepted && !item?.isRejected && (
                            <button
                              onClick={() => {
                                handleReq(item?._id, "accept");
                              }}
                              className=" normalButtonTag bg-green-500 w-[150px] float-right relative"
                            >
                              Accept
                            </button>
                          )}
                          {!item?.isRejected && !item?.isAccepted && (
                            <button
                              onClick={() => {
                                handleReq(item?._id, "reject");
                              }}
                              className=" normalButtonTag bg-red-600 w-[150px] float-right relative mx-4"
                            >
                              Reject
                            </button>
                          )}
                        ...
                        {/* Request To User  */}
                        <div className=" w-1/5 ">
                          <p className=" mt-3 text-lg">Request by</p>
                          <UserShortView user={item?.requestBy[0]} />
                        </div>
                      </div>
                    )}
                  </div>
